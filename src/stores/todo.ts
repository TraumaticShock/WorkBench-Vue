  ...
  const actions = {
    ...
    async toggleTodoStatus(id: string, currentStatus: 'completed' | 'pending') {
      try {
        ...
        // 更新本地状态
        const index = state.value.todoPage.records.findIndex(
    ...
    async updateTodo(id: string, todoData: CreateTodoFormParams) {
      try {
        ...
        if (index !== -1) {
          state.value.todoPage.records[index] = {
            ...state.value.todoPage.records[index],
            ...todoData,
          };
        }
    ...
    async deleteTodo(id: string) {
      try {
        ...
        // 更新本地状态
        state.value.todoPage.records = state.value.todoPage.records.filter(
          (todo) => todo.id.toString() !== id
        );
    ...
    async refreshStats() {
      try {
        // 获取总数统计
        const { data: totalData } = await todoApi.getTodoStatistics({
          unit: 'day',
          date: getTodayString(),
        });
        // 获取未完成统计
        const { data: uncompleteData } = await todoApi.getTodoStatistics({
          unit: 'day',
          status: 'pending',
          date: getTodayString(),
        });
        // 获取已完成统计
        const { data: completeData } = await todoApi.getTodoStatistics({
          unit: 'day',
          status: 'completed',
          date: getTodayString(),
        });
        // 获取优先级统计
        const { data: highData } = await todoApi.getTodoStatistics({
          unit: 'day',
          date: getTodayString(),
          priority: 'high',
        });
        const { data: mediumData } = await todoApi.getTodoStatistics({
          unit: 'day',
          date: getTodayString(),
          priority: 'medium',
        });
        const { data: lowData } = await todoApi.getTodoStatistics({
          unit: 'day',
          date: getTodayString(),
          priority: 'low',
        });
        // 获取本周已完成统计
        const { data: weekCompleteData } = await todoApi.getTodoStatistics({
          unit: 'week',
          status: 'completed',
        });
        // 获取分类统计
        const { data: categoryData } = await todoApi.getTodoStatistics({
          unit: 'category',
          status: 'pending',
        });
        ...
        state.value.stats = {
          ...
          categoryCount: processCategoryData(
    ...
    // 刷新所有待办相关数据
    async refreshAllTodoData() {
      await Promise.all([
        this.getTodoPage({ page: 1, size: 10 }),
        this.refreshStats(),
      ]);
    },
  };
