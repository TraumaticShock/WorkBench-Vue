                ...
                <div class="w-1/2 border-r border-base-200 overflow-y-auto">
                    <div class="p-4">
                        ...
                        <div v-else class="category-tree">
                            <NoteCategoryTreeItem v-for="category in categories" :key="category.id" :category="category"
...
const flatCategories = computed(() => {
    const result: NoteCategory[] = []
    const flatten = (items: NoteCategory[]) => {
        items.forEach(item => {
            result.push(item)
...
// 选择分类
const selectCategory = (category: NoteCategory) => {
    selectedCategory.value = { ...category }
    isCreating.value = false
}

// 展开/折叠分类
const toggleExpand = (category: NoteCategory) => {
...
const handleSave = async () => {
    ...
    try {
        if (isCreating.value) {
            // 新建分类
            await noteCategoryStore.actions.createCategory(selectedCategory.value.name, selectedCategory.value.parentId);
        } else {
            // 更新分类
            await noteCategoryStore.actions.updateCategory(selectedCategory.value.id, selectedCategory.value);
        }
...
const categoryToDelete = ref<NoteCategory | null>(null)

// 修改删除处理函数
const handleDelete = async (category: NoteCategory) => {
    ...
    if (hasChildren) {
        ...
        // 直接删除没有子分类的分类
        noteCategoryStore.actions.deleteCategory(category.id)
        selectedCategory.value = null
    }
}
...
const confirmDelete = () => {
    if (categoryToDelete.value) {
        noteCategoryStore.actions.deleteCategory(categoryToDelete.value.id)
        selectedCategory.value = null
