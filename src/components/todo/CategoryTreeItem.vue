<template>
    <div class="category-item">
        <div class="flex items-center gap-2 p-2 rounded-lg hover:bg-base-200 cursor-pointer group"
            ...
            <!-- 展开/折叠按钮 -->
            <button v-if="category.children?.length" 
                class="btn btn-xs btn-ghost btn-square"
                @click.stop="$emit('toggle', category)">
                <svg xmlns="http://www.w3.org/2000/svg" 
                    :class="['h-4 w-4 transition-transform', { 'rotate-90': category.expanded }]"
        ...
        <!-- 子分类 -->
        <div v-if="category.children?.length && category.expanded" 
            class="ml-6 mt-1 border-l-2 border-base-200 pl-2">
            <CategoryTreeItem
